...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <form id="api-scope-group-form" name="api-scope-group-form" action='{{URL::action("ApiScopeGroupController@update",null)}}'>
...
@section('scripts')
    <script type="application/javascript">
        ...
        @foreach($non_selected_users as $user)
            @if($user->email !== false)
                all_users.push({id: {{$user->id}}, value: '{{$user->email}}' });
            @endif
        @endforeach
        var current_scopes = [];
        var current_users  = [];
        ...
        @foreach($group->users()->get() as $user)
            current_users.push({ "id": {{$user->id}} , "value": "{{$user->email}}" });
        @endforeach
