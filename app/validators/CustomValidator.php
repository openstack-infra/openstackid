...
class CustomValidator extends Validator
{
    ...
    public function validateCustomUrlSet($attribute, $value, $parameters)
    {
        ...
        $res = true;
        foreach ($urls as $url) {
            $res = $app_type === IClient::ApplicationType_Native ? $this->validateUrl($attribute, $url) : $this->validateSslurl($attribute, $url);
            if (!$res) {
                break;
            }
        }
    ...
    public function validateKeyAlg($attribute, $value, $parameters)
    {
        ...
        $res = in_array($value, OAuth2Protocol::$supported_algorithms);
