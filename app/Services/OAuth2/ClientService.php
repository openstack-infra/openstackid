...
class ClientService implements IClientService
{
    ...
    public function unlockClient($id)
    {
        return $this->tx_service->transaction(function () use ($id) {
            ...
            if (is_null($client)) {
                throw new AbsentClientException($id, sprintf("client id %s does not exists!", $id));
            }
            $client->locked = false;
            $this->client_repository->update($client);
            return true;
    ...
    {
        return Client::Filter($filters)->paginate($page_size, $fields, $pageName ='Page', $page_nbr);
    }
