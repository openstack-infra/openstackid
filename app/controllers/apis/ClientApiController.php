 ...
 * Client REST API
 */
class ClientApiController extends JsonController implements IRESTController
{
    ...
    public function updateStatus($id)
    {
        try {
            ...
            $res = $this->client_service->activateClient($id, $values['active']);
    ...
    public function revokeToken($id, $value, $hint)
    {
        try {
            ...
            switch ($hint) {
                case 'access-token':{
                    $token = $this->token_service->getAccessToken($value);
                    if(is_null($token)){
                ...
                break;
                case 'refresh-token':{
                    $token = $this->token_service->getRefreshToken($value);
                    if(is_null($token)){
