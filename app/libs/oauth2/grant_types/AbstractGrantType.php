...
abstract class AbstractGrantType implements IGrantType
{
    ...
    protected function shouldPromptLogin(OAuth2AuthorizationRequest $request)
    {
        if($request instanceof OAuth2AuthenticationRequest)
        {
            ...
            if(in_array(OAuth2Protocol::OAuth2Protocol_Prompt_Login, $request->getPrompt()))
                return true;
        }
        return false;
    }
    ...
    protected function shouldPromptConsent(OAuth2AuthorizationRequest $request)
    {
        ...
        (
            ...
            !$this->security_context_service->get()->isConsentPromptProccessed()
        )
        {
            $this->security_context_service->save
            (
                $this->security_context_service->get()->setConsentPromptProccessed(true)
            );
            return true;
        }
        return false;
    ...
    protected function mustAuthenticateUser(OAuth2AuthorizationRequest $request, IClient $client)
    {
        ...
        if($this->shouldPromptLogin($request) && !$this->security_context_service->get()->isLoginPromptProccessed())
        {
            $this->security_context_service->save
            (
                $this->security_context_service->get()->setLoginPromptProccessed(true)
            );
            $this->auth_service->logout();
            return true;
        }
