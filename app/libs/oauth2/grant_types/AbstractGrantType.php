...
abstract class AbstractGrantType implements IGrantType
{
    ...
    public function completeFlow(OAuth2Request $request)
    {
        ...
        if (is_null($this->current_client))
            throw new InvalidClientException($this->client_auth_context->getId(), sprintf("client id %s does not exists!",$this->client_auth_context->getId()));

        if (!$this->current_client->isActive() || $this->current_client->isLocked()) {
            throw new LockedClientException($this->client_auth_context->getId(), sprintf('client id %s', $this->client_auth_context->getId()));
        }
        ...
        if(!ClientAuthContextValidatorFactory::build($this->client_auth_context)->validate($this->client_auth_context))
            throw new InvalidClientCredentials($this->client_auth_context->getId(), sprintf('client id %s',$this->client_auth_context->getId()));
