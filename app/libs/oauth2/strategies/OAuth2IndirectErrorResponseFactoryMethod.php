        ...
        if($request instanceof OAuth2AuthorizationRequest)
        {
            ...
            switch($response_type)
            {
                case OAuth2Protocol::OAuth2Protocol_ResponseType_Token:
                    return new OAuth2IndirectFragmentErrorResponse($error, $return_url, $request->getState());
                    break;
                case OAuth2Protocol::OAuth2Protocol_ResponseType_Code:
                    return new OAuth2IndirectErrorResponse($error, $return_url, $request->getState());
                    break;
                default:
