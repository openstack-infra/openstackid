...
Route::group(array("before" => array("ssl", "auth")), function () {
    ...
    Route::get('/accounts/user/profile/clients/edit/{id}',array('before' => 'user.owns.client.policy', 'uses' => 'UserController@getEditRegisteredClient'));
 });

Route::group(array('prefix' => 'admin','before' => 'ssl|auth|auth.server.admin'), function(){
...
Route::group(array('prefix' => 'admin/api/v1', 'before' => 'ssl|auth'), function()
{
    ...
    Route::group(array('prefix' => 'clients'), function(){
        ...
        Route::get('/{user_id}/{page_nbr}/{page_size}',array('before' => 'is.current.user', 'uses' => 'ClientApiController@getByPage'));
