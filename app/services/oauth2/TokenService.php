...
class TokenService implements ITokenService
{
    ...
    public function __construct(IClientService $client_service, ILockManagerService $lock_manager_service, IServerConfigurationService $configuration_service, ICacheService $cache_service, IAuthService $auth_service)
    {
    ...
    {
        //create model
        $code         = AuthorizationCode::create($user_id,$client_id, $scope, $audience, $redirect_uri, $this->configuration_service->getConfigValue('OAuth2.AuthorizationCode.Lifetime'));
        $value        = $code->getValue();
    ...
    public function getAuthorizationCode($value)
    {
        ...
        try {
            ...
            $cache_values = $this->cache_service->getHash($hashed_value, array('user_id','client_id','scope','audience','redirect_uri','issued','lifetime','from_ip'));
    ...
    public function createAccessTokenFromParams($client_id,$scope, $audience,$user_id = null)
    {
        ...
        DB::transaction(function () use ($client_id,$scope, $audience,$user_id, &$access_token) {
    ...
    public function getAccessTokenByUserId($user_id){
        $user         = $this->auth_service->getClientById($user_id);
        if(is_null($user))
